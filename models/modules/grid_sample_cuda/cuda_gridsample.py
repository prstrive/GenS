...
gridsample_grad2 = cpp_extension.load(name='gridsample_grad2', sources=['models/modules/grid_sample_cuda/gridsample_cuda.cpp', 'models/modules/grid_sample_cuda/gridsample_cuda.cu'], verbose=True)
...
class _GridSample3dBackward(torch.autograd.Function):
    ...
    def backward(ctx, grad2_grad_input, grad2_grad_grid):
        grad_output, input, grid = ctx.saved_tensors
        if grad2_grad_input is None:
            grad2_grad_input = torch.zeros_like(input)
        assert grad_output.is_cuda and input.is_cuda and grid.is_cuda and grad2_grad_input.is_cuda and grad2_grad_grid.is_cuda
