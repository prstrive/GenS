...
class DTUDatasetFinetune(Dataset):
    def __init__(self, confs, mode):
        ...
        self.pseudo_dense_lis = [os.path.join(self.data_dir, 'pseudo_depths/{}/{}_epoch0.npy'.format(self.scene, vid)) for vid in self.all_views]
        ...
        self.dense_pseudo_depths = [np.load(filename).astype(np.float32) / self.pseudo_scale for filename in self.pseudo_dense_lis]
        # self.dense_pseudo_depths = np.stack([cv2.resize(img, self.img_hw[::-1], interpolation=cv2.INTER_NEAREST) for img in self.dense_pseudo_depths])
        self.dense_pseudo_depths = np.stack(self.dense_pseudo_depths)
        self.dense_pseudo_depths = torch.from_numpy(self.dense_pseudo_depths.astype(np.float32)) * self.scale_factor
        
        pseudo_ptses = []
        for i in range(self.num_views):
            dense_pseudo_depth = self.dense_pseudo_depths[i]
            p_mask = (dense_pseudo_depth > 0)
            d_h, d_w = dense_pseudo_depth.shape
            y, x = torch.meshgrid(torch.arange(0, d_h), torch.arange(0, d_w))
            x = x[p_mask]
            y = y[p_mask]
            p_depth = dense_pseudo_depth[p_mask]
            intr = self.intrs[i].clone()
            intr[0] *= d_w / self.img_hw[1]
            intr[1] *= d_h / self.img_hw[0]
            xyz_ref = torch.matmul(intr.inverse()[:3, :3], torch.stack((x, y, torch.ones_like(x)), dim=0) * p_depth.unsqueeze(0))
            xyz_world = torch.matmul(self.c2ws[i], torch.cat((xyz_ref, torch.ones_like(x).unsqueeze(0)), dim=0))[:3]
            pseudo_pts = xyz_world.permute(1, 0)
            pseudo_ptses.append(pseudo_pts)
        self.pseudo_ptses = torch.cat(pseudo_ptses, dim=0)
    ...
    def get_random_rays(self, vid):
        ...
        random_idx = torch.randint(low=0, high=self.pseudo_ptses.shape[0], size=[2048])
        pseudo_pts = self.pseudo_ptses[random_idx]
