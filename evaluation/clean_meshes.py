...
def clean_mesh_faces_outside_frustum(args, scan, old_mesh_file, new_mesh_file, imgs_idx, H=1200, W=1600, mask_dilated_size=11,
    ...
    for i in view_idx:
        ...
        for rays_o_batch, rays_d_batch, rays_mask_batch in tqdm(zip(rays_o, rays_d, rays_mask)):
            ...
            rays_o_batch = rays_o_batch + rays_d_batch * dep_min
    ...
    mesh_tri = trimesh.load(old_mesh_file)
    mesh_tri.update_faces(mask_faces == 0)
    # mesh_tri.export(new_mesh_file.replace(".ply", "_raw.ply"))  # for save
    mesh_tri.export(new_mesh_file)  # will be replace
    print('save to {:s}'.format(new_mesh_file))
...
if __name__ == "__main__":
    ...
    if args.set==0:
        ...
        view_list = [43, 33, 44, 42, 34, 32, 45, 23, 41, 24, 31]
